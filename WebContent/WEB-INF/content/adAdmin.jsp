...
<div class="pd-20">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
			<s:iterator value="#request.result" id="admin">
				<tr class="text-c">
					<td>${admin.id}</td>
					<td>${admin.logName}</td>
					<td>${admin.level}</td>
					<td>${admin.state}</td>
					<td class="td-manage"><a href="adAdmin_stateChange?id=${admin.id}">${admin.state==AdminState.正常?'禁用':'启用'}</a> <a href="adAdmin_delete?id=${admin.id}">删除</a></td>
				</tr>
			</s:iterator>
			</tbody>
		</table>
